...
(() => {
  ...
  // And finally, disable throwOnModuleCollision -- this causes all kinds of problems and
  // solves none.
  try {
    fs.writeFileSync(local_path + 'react-native/node_modules/metro/src/node-haste/DependencyGraph.js',
      fs.readFileSync(path.resolve(__dirname, 'metro-polyfill/DependencyGraph.js'))
    );
  } catch (e) {
    fs.writeFileSync(local_path + 'metro/src/node-haste/DependencyGraph.js',
      fs.readFileSync(path.resolve(__dirname, 'metro-polyfill/DependencyGraph.js'))
    );
  }
